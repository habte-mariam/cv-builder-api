...
if st.session_state.page == "Dashboard":
    ...
    with st.container(border=True):
        ...
        st.write(
            "Enter your email address to retrieve and manage your saved resumes.")
                    ...
                    for user in res.data:
                        with st.container(border=True):
                            ...
                            with c1:
                                ...
                                st.caption(
                                    f"🎯 **Role:** {user.get('job_title', 'Not Specified')} | 📍 {user.get('address', '')}")
...
# --- 4. Create/Edit Page ---
    ...
    if not ui:
        st.info(
            "💡 Please start by loading a CV from the Dashboard or enter your details below.")
...
# --- የተስተካከለ የ Age Logic (በካላንደር) ---
            ...
            birth_date = st.date_input(
                ...
                max_value=date(today.year - 18, today.month,
                               today.day),  # ከ18 አመት በታች መሆን አይችልም
                min_value=date(today.year - 65, today.month,
                               today.day)  # ከ65 አመት በላይ መሆን አይችልም
            )
        ...
        with tabs[1]:
            ...
            gy = st.number_input("Year of Completion", 1990,
                                 2030, int(ed.get('grad_year', 2024)))
        ...
        with tabs[2]:
            ...
            try:
                raw_dur = int(
                    ''.join(filter(str.isdigit, str(ex.get('duration', '0')))))
            except:
                raw_dur = 0
...
# 5. Skills Tab
        with tabs[4]:
            ...
            if 'temp_skills' not in st.session_state:
                st.session_state.temp_skills = [s['name'] for s in ui.get(
                    'skills', [])] if isinstance(ui.get('skills'), list) else []

            # 2. የክህሎት ዘርፉን ማስመረጥ
            skill_categories = list(SKILLS_DATABASE.keys())
            selected_cat = st.selectbox("Choose Skill Category", options=[
                                        "All"] + skill_categories)

            # 3. እንደ ተመረጠው ዘርፍ ዝርዝሩን ማጣራት
            if selected_cat == "All":
                category_options = [
                    skill for sublist in SKILLS_DATABASE.values() for skill in sublist]
            else:
            ...
            all_options = sorted(
                list(set(category_options + st.session_state.temp_skills)))
            ...
            current_selections = st.multiselect(
                ...
                options=all_options,
                default=[
                    s for s in st.session_state.temp_skills if s in all_options]
            )

            # 6. ሴሽን ስቴቱን ማዘመን
            # አዲስ የተመረጡትን መያዝ እና የተሰረዙትን ማስወገድ
            updated_skills = list(
                set(st.session_state.temp_skills + current_selections))
        ...
        with tabs[5]:
            ...
            submit = st.form_submit_button(
                "🚀 Save Data & Generate Full Preview", use_container_width=True)
